      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {properties.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No properties found for sale.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {properties.map((property) => (
              <Link
                key={property.id}
                href={`/buypage/listing-flow?id=${property.id}`}
              ...
              >
                ...
                <div className="relative h-64 bg-gray-200">
                  {property.property_images && property.property_images.length > 0 ? (
                    <>
                      <img
                        src={`/buyrentkenya_data/${property.property_images[0].image_path}`}
                        alt={property.title}
                      ...
                      {property.property_images.length > 1 && (
                        <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 flex space-x-1">
                          {property.property_images.slice(0, 5).map((_, index) => (
                            <div
                              key={index}
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center text-sm text-gray-600 mb-3">
                    ...
                    <span className="mx-2 text-gray-400">-</span>
                    <span className="text-gray-500">{property.property_type} for {property.transaction_type.toLowerCase()}</span>
                  </div>
